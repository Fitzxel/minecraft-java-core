...
export default class JavaDownloader extends EventEmitter {
  ...
  public async getJavaFiles(
    jsonversion: MinecraftVersionJSON
  ): Promise<JavaDownloadResult> {
    ...
    const archMapping: Record<string, Record<string, string>> = {
      ...
      darwin: {
        x64: "mac-os",
        arm64: this.options.intelEnabledMac ? "mac-os" : "mac-os-arm64",
      },
    ...
    const versionName =
      javaVersionsJson[archOs]?.[javaVersionName]?.[0]?.version?.name;
    if (!versionName) {
      return this.getJavaOther(jsonversion);
    }
    ...
    const manifestUrl =
      javaVersionsJson[archOs][javaVersionName][0]?.manifest?.url;
    const manifest = await fetch(manifestUrl).then((res) => res.json());
    const manifestEntries: Array<[string, any]> = Object.entries(
      manifest.files
    );
    ...
    const javaEntry = manifestEntries.find(([relPath]) =>
      relPath.endsWith(javaExeKey)
    );
    if (!javaEntry) {
    ...
    for (const [relPath, info] of manifestEntries) {
      ...
      files.push({
        path: `runtime/jre-${versionName}-${archOs}/${relPath.replace(
          toDelete,
          ""
        )}`,
   ...
   */
  public async getJavaOther(
    jsonversion: MinecraftVersionJSON,
    versionDownload?: string
  ): Promise<JavaDownloadResult> {
    const { platform, arch } = this.getPlatformArch();
    const majorVersion =
      versionDownload || jsonversion.javaVersion?.majorVersion || 8;
    const pathFolder = path.resolve(
      this.options.path,
      `runtime/jre-${majorVersion}`
    );

    // Build the API query to fetch the Java version from Adoptium
    const imageType = this.options.java.type; // 'jdk' or 'jre'
    ...
    const javaVersionURL = `https://api.adoptium.net/v3/assets/feature_releases/${majorVersion}/ga?${queryParams.toString()}`;

    let javaResponse = await fetch(javaVersionURL).then((res) => res.json());
    if (
      !Array.isArray(javaResponse) ||
      javaResponse.length === 0 ||
      !javaResponse[0].binaries ||
      javaResponse[0].binaries.length === 0
    ) {
      return {
        files: [],
        path: "",
        error: true,
        message: "No Java versions found for the specified parameters.",
      };
    }

    // Get the first binary from the response
    const binary = javaResponse[0].binaries[0];
    const packageInfo = binary.package;
    const downloadUrl = packageInfo.link;
    const packageName = packageInfo.name;

    // Determine the expected Java executable path
    // For tar.gz files, we need to find the actual extracted directory name
    const isCompressed =
      packageName.endsWith(".tar.gz") || packageName.endsWith(".tgz");

    // Helper function to find Java executable
    const findJavaExecutable = (): string | null => {
      if (!fs.existsSync(pathFolder)) return null;

      const dirs = fs.readdirSync(pathFolder).filter((item) => {
        const itemPath = path.join(pathFolder, item);
        return (
          fs.statSync(itemPath).isDirectory() &&
          item !== packageName.replace(".zip", "").replace(".tar.gz", "")
        );
      });
      ...
      for (const dir of dirs) {
        if (platform === "mac") {
          ...
          const winPath = path.join(pathFolder, dir, "bin", "java.exe");
          const winPathW = path.join(pathFolder, dir, "bin", "javaw.exe");
          if (fs.existsSync(winPath)) return winPath;
          if (fs.existsSync(winPathW)) return winPathW;
        } else {
          const linuxPath = path.join(pathFolder, dir, "bin", "java");
    ...
    let javaExePath = findJavaExecutable();

    if (!javaExePath) {
      await this.verifyAndDownloadFile({
        filePath: path.join(pathFolder, packageName),
        pathFolder: pathFolder,
        fileName: packageName,
        url: downloadUrl,
      });

      const entries = await getFileFromArchive(
        path.join(pathFolder, packageName),
        null,
        null,
        true
      );
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory) {
          fs.mkdirSync(`${pathFolder}/${entry.name}`, {
            recursive: true,
            mode: 0o777,
          });
          continue;
        }
        fs.writeFileSync(`${pathFolder}/${entry.name}`, entry.data, {
          mode: 0o777,
        });
      }

      // Find the Java executable after extraction
      javaExePath = findJavaExecutable();
