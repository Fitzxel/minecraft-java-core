...
export type MicrosoftClientType = "electron" | "nwjs" | "terminal" | "raw";
...
export default class Microsoft {
   ...
   * @param prompt    The OAuth prompt behavior (default: 'select_account').
   * @param redirect  The OAuth redirect URI (default: Microsoft's desktop redirect).
   */
  constructor(
    client_id: string,
    prompt: string,
    ...
    this.prompt = prompt || "select_account";
    this.redirect = redirect || "https://login.live.com/oauth20_desktop.srf";
  ...
  private async getAccount(oauth2: any): Promise<AuthResponse | AuthError> {
    ...
    const mcLoginResponse = await fetch(
    ...
    const mcstoreResponse = await fetch(
    ...
    const xboxAccountResponse = await fetch(
      "https://xsts.auth.xboxlive.com/xsts/authorize",
      {
        method: "POST",
  ...
  public async getProfile(mcLogin: {
    access_token: string;
