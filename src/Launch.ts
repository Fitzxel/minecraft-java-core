...
export default class Launch extends EventEmitter {
  ...
  async config(opt: LaunchOPTS) {
    ...
    if (this.options.mcp) {
      if (this.options.instance)
        ...
        this.options.mcp = path
          .resolve(`${this.options.path}/${this.options.mcp}`)
          .replace(/\\/g, "/");
    }
  ...
  /**
   * Download and set up game data. Can be called independently.
   */
  async downloadGame() {
    if (!this.options)
      return this.emit("error", {
        error: "Options not set. Call Config() first.",
      });

    // Define save path
    let saveDir = this.options.instance
    ...
    if (!hasInternet) {
      ...
      if (fs.existsSync(savePath)) {
        try {
          ...
          this.gameData = data;
          this.emit("data", "Sin conexión: usando gameData.json local");
          return this.gameData;
        } catch (e) {
          return this.emit("error", {
            error: "No hay conexión y el archivo gameData.json está corrupto.",
          });
        }
      } else {
        return this.emit("error", {
          error: "No hay conexión y no existe gameData.json local.",
        });
      }
    }
    ...
    this.emit("gameDownloadFinished");
    ...
    fs.writeFileSync(savePath, JSON.stringify(this.gameData, null, 2), "utf-8");

    return this.gameData;
  }

  /**
   * Starts the game. Can be called independently.
   * @param gameData Optional game data object. If not provided, uses previously downloaded data or downloads it.
   */
  async start(gameData?: any) {
    let data = gameData || this.gameData || (await this.downloadGame());
    if (!data || data.error)
      return this.emit("error", data || { error: "No game data available" });
    let { minecraftJson, minecraftLoader, minecraftVersion, minecraftJava } =
      data;
    ...
    let java: any = this.options.java.path
      ? this.options.java.path
      : minecraftJava.path;
    let logs = this.options.instance
    ...
    let minecraftDebug = spawn(java, Arguments, {
      cwd: logs,
